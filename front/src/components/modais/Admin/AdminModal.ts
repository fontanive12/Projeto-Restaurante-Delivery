...
import { DivContainer, ItemsFormContainer } from "./AdminModal.styles";
...
export function AdminModal({ closeModal, userData }: UserModalProps) {
...
async function handleCrateNewAdmin(data: Admin) {
  console.log("acessou");
  try {
    console.log(data);
    if (data.id !== 0) {
      console.log("acessou");
      await axios.put(`http://localhost:3000/admins/${data.id}`,
        data)
...
export const showAdminCreateBox = async () => {
  Swal.fire({
    title: 'Create admin',
    html:
      '<form id="swal-form">' +
      ...
      '<input id="birth_date" name="birth_date" class="swal2-input" placeholder="Data de nascimento">' +

      '<input id="genre" name="genre" class="swal2-input" placeholder="Gênero" >' +

      '<input id="phoneNumber" name="phoneNumber" class="swal2-input" placeholder="Número de telefone">' +
      
      '<input id="position" name="position" class="swal2-input" placeholder="Cargo">' +
...
export const showUserEditBox = async (data: Admin) => {
  Swal.fire({
    title: 'Admin city',
    html:
      '<form id="swal-form">' +
